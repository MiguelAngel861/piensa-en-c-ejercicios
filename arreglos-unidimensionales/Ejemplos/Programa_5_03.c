...
int main(void)
{
        ...
        *IX = 15; /* Z[2] se modifica, ahora vale 15. */
        ...
        X = *IX + 5; /* XX se modifica, ahora vale Z[2] + 5 = 20. Recuerda que *IX contiene el valor de Z[2]. */
        ...
        *IX += 1; /* Z[2] se vuelve a modificar, ahora vale 12. */
        ...
        X = *(IX + 1); /* XX se modifica. El apuntador IX accede temporalmente a Z[3], por lo tanto X toma este valor (8). Observa que IX no se reasigna */
        Y = *IX; /* Y se modifica, toma el valor de Z[2] (12). */

        printf("\nX = %d \t Y = %d \t Z[0] = %d \t Z[1] = %d \t Z[2] = %d \t Z[3] = %d\t Z[4] = %d", X, Y, Z[0], Z[1], Z[2], Z[3], Z[4]);
        
        IX = IX + 1; /* IX se modifica. cdObserva la forma de mover el apuntador. Ahora IX apunta al cuarto elemento de Z (Z[3]). */
        ...
        Y = *IX; /* YY se modifica, toma el valor (basura) de una celda incorrecta. Es un error que no señala el compilador del lenguaje C. */
