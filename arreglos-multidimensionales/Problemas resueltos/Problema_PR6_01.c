 ...
 * (matriz) cuadrado, determine si el mismo es simétrico. Recuerda que se 
 * considera simétrico si cumple la siguiente condición: A[I][J] = A[J][I].
 * 
...
/* 
 * Simétrico.
 * El programa, al recibir como dato un arreglo bidimensional
 * cuadrado, determina si el mismo es simétrico. 
 */
...
{
        /* La función Lectura se utiliza para leer 
         * un arreglo bidimensional cuadrado de
...
int Simetrico (int A[][MAX], int T)
{
         ...
         * Observe que en el segundo ciclo solamente se recorre la matriz triangular
